  ...
  protected render(): TemplateResult {
    return html`<div class="relative">
      <div
        ...
        class="rounded bg-black p-1 text-xs"
      >
        ${this.buttonText}
      </div>
      ${this.show
        ? html`<div
            @click=${() => (this.show = !this.show)}
            class="absolute bg-black p-4 rounded border border-gray/50"
          >
            <slot></slot>
          </div>`
        : nothing}
  ...
  const galleryDom = dom(html`
    <div class="flex flex-col gap-2 mt-2">
      ${images.map(
        (img, index) => html`
          <div
            class="relative flex flex-col items-center ${index && !expandGallery
              ? "hidden"
              : ""}"
          >
            <img
              class="max-h-[70vh] border border-none rounded"
              src="${img.thumb}"
              alt="${img.alt}"
              )
            />
            ${img.alt && img.alt.length > 0
              ? html`<popup-overlay
                  buttonText="ALT"
                  text="${img.alt}"
                  class="absolute left-1 bottom-1 cursor-pointer"
                >
      ...
      ${images.length > 1 && !expandGallery
        ? html`<div
            ...
            @click=${(ev: Event) => {
              imageDoms[0].click();
              (ev.target as HTMLElement).innerText = `Show ${
                images.length - 1
              } more images`;
            }}
          >
...
export function renderCard(card: BskyExternalCard) {
  return html` <a
    href="${card.uri}"
    class="inline-block w-full border border-gray/50 rounded mt-2"
    target="_blank"
  >
    <div class="flex">
      ${card.thumb
        ? html`<div>
            <img
              src="${card.thumb}"
              class="!w-[240px] !max-h-full !h-full !object-cover !rounded-r-none"
            />
          </div>`
        : nothing}
      <div class="flex flex-col p-4 overflow-hidden">
        <span class="font-bold text-sm text-color"
          >${card.title ? card.title : card.uri}</span
        >
        <span class="py-2 text-color text-sm text-ellipsis overflow-hidden"
          >${card.description.split("\n")[0]}</span
        >
        <span class="text-xs text-color/50 text-ellipsis overflow-hidden"
          >${new URL(card.uri).host}</span
        >
      </div>
    </div>
  </a>`;
...
export const contentLoader = html`<div
  class="flex space-x-4 animate-pulse w-[80%] max-w-[300px] m-auto py-4"
>
...
function icon(svg: string) {
  return html`<i class="flex w-[1.2m] h-[1.2em] border-white fill-primary"
    >${unsafeHTML(svg)}</i
  >`;
}
...
export class ThemeToggle extends LitElement {
  ...
  render() {
    ...
    return html`<button
      class="absolute top-0 right-0 p-4 fill-primary"
